...
## Create Cytoscape network for the RWR results
## WF Environment 
...
#---------------------------------------------------------------
## CREATE NETWORK FROM GR FILE
##
## INPUT :
##    - networkFileName : [STRING] path of network file
## OUTPUT :
##    - network : [iGraph] network
#---------------------------------------------------------------
createNetwork <- function(networkFileName){
  network_table <- read.table(file = networkFileName, sep = "\t")
  network <- igraph::graph.data.frame(d = network_table, directed = FALSE)
...
##
## INPUT :
##    - bipartiteFileName : [STRING] path of bipartite file
## OUTPUT :
##    - bipartite_network : [iGraph] network
#---------------------------------------------------------------
readBipartite <- function(bipartiteFileName){
  bipartite_table <- read.table(file = bipartiteFileName, sep = "\t")
  bipartite_network <- igraph::graph.data.frame(d = bipartite_table, directed = FALSE)
  bipartite_network <- igraph::simplify(graph = bipartite_network, remove.multiple = TRUE, remove.loops = TRUE)
...
#---------------------------------------------------------------
## CREATE CYTOSCAPE SESSION AND CUSTOM NETWORKS
##
## INPUT :
##    - cytoscapeSessionName : name of cytoscape session (existing or those you want)
